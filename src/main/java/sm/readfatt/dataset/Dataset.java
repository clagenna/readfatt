...
public class Dataset {
  ...
  public void addRegexRiga(String p_rex, String pCivetta) {
    if (m_liGestRiga == null) m_liGestRiga = new ArrayList<GestRiga2>();
  ...
  public void parseRiga(String p_sz) {
    for (GestRiga2 gr : m_liGestRiga) {
      if (gr.parseRiga(p_sz.replace("\r", ""))) break;
    }
  }
      ...
      for (DtsRow ri : righe) {
        ...
        for (DtsData da : ri.getDati()) {
          ...
          if (da != null) {
            ...
            if (obj instanceof Date) sz = Utils.s_fmtDMY4.format((Date) obj);
             else sz = String.valueOf(da.getDato());
          }
          pwr.print(virg + sz);
          virg = ";";
        }
  ...
  public Object getValue(String string, int i) {
    int k = i;
    if (k < 0) k = righe.size() - 1;
    if (k > righe.size() - 1) throw new ArrayIndexOutOfBoundsException(k);
