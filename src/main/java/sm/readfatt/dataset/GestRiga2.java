...
public class GestRiga2 {
  ...
  private static final org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(GestRiga2.class);
  ...
  private void addField(GestDato2 gd, boolean b) {
    if (m_liGestDato == null) m_liGestDato = new ArrayList<GestDato2>();
    if (b) m_liGestDato.add(0, gd);
     else m_liGestDato.add(gd);
  }

  public boolean parseRiga(String p_sz) {
    boolean bRet = false;
    String szDebug = "Credito precedente";
    if (civetta != null) if (!p_sz.matches(".*" + civetta + ".*")) return bRet;
    Matcher ma = m_patt.matcher(p_sz);
    if (p_sz.contains(szDebug)) log.debug("Trovato \"{}\"", p_sz);
    bRet = ma.find();
    if (!bRet) {
      if (civetta != null) log.error("frase:\"{}\" ma ... \"{}\" no match !", civetta, p_sz);
      return bRet;
    }
    int k = 1;
  ...
  public String toString() {
    String szRet = "*null*";
    if (m_liGestDato == null || m_liGestDato.size() == 0) return szRet;
    szRet = "";
