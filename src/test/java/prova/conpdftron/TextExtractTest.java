...
public class TextExtractTest {
  ...
  public static void main(String[] args) {
    ...
    String  input_path         = "F:/java/photon2/readfatt/dati/";
    ...
    try {
      PDFDoc doc = new PDFDoc("F:/java/photon2/readfatt/dati/EE_202201_1000885.pdf");
      ...
      if (example2_xml) {
        String text = txt
            .getAsXML(TextExtractor.e_words_as_elements | TextExtractor.e_output_bbox | TextExtractor.e_output_style_info);
        System.out.println("\n\n- GetAsXML  --------------------------\n" + text);
        System.out.println("-----------------------------------------------------------");
      }
      ...
      if (example4_advanced) {
        ...
        for (line = txt.getFirstLine(); line.isValid(); line = line.getNextLine()) {
          ...
          System.out.print("<Line box=\""
              + String.format("%.2f, %.2f, %.2f, %.2f", bbox.getX1(), bbox.getY1(), bbox.getX2(), bbox.getY2()) + "\"");
          TextExtractTest.printStyle(line_style);
    ...
    if (example5_low_level) {
      try {
        ...
        for (PageIterator itr = doc.getPageIterator(); itr.hasNext();) {
          ...
          TextExtractTest.DumpAllText(reader);
          reader.end();
        }
        ...
        String s1         = TextExtractTest.ReadTextFromRect(first_page, new Rect(27, 392, 563, 534), reader);
        System.out.print("\nField 1: " + s1);

        s1 = TextExtractTest.ReadTextFromRect(first_page, new Rect(28, 551, 106, 623), reader);
        System.out.print("\nField 2: " + s1);

        s1 = TextExtractTest.ReadTextFromRect(first_page, new Rect(208, 550, 387, 621), reader);
        System.out.print("\nField 3: " + s1);
  ...
  static void printStyle(TextExtractor.Style s) {
    ...
    System.out.print(" style=\"font-family:" + s.getFontName() + "; " + "font-size:" + df.format(s.getFontSize()) + ";"
    ...
    while ( (element = reader.next()) != null) {
      switch (element.getType()) {
        ...
        case Element.e_text: {
          ...
          if (bbox == null)
            continue;
          System.out.println("\n--> BBox: " + bbox.getX1() + ", " + bbox.getY1() + ", " + bbox.getX2() + ", " + bbox.getY2());
  ...
  static String RectTextSearch(ElementReader reader, Rect pos) throws PDFNetException {
    ...
    while ( (element = reader.next()) != null) {
      switch (element.getType()) {
        ...
        {
          reader.formBegin();
          srch_str += TextExtractTest.RectTextSearch(reader, pos);
          reader.end();
          break;
        }
      }
  ...
  static String ReadTextFromRect(Page page, Rect pos, ElementReader reader) throws PDFNetException {
    reader.begin(page);
    String srch_str = TextExtractTest.RectTextSearch(reader, pos);
    reader.end();
    return srch_str;
  }
}
